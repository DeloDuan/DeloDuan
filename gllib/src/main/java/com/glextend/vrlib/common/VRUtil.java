...
import com.glextend.vrlib.model.GLVector3D;
import com.glextend.vrlib.model.GLRay;
...
public class VRUtil {
    ...
    public static GLVector3D vec3Sub(GLVector3D v1, GLVector3D v2){
        return new GLVector3D().setX(v1.getX() - v2.getX()).setY(v1.getY() - v2.getY()).setZ(v1.getZ() - v2.getZ());
    }

    public static GLVector3D vec3Cross(GLVector3D v1, GLVector3D v2){
        return new GLVector3D().setX(v1.getY() * v2.getZ() - v2.getY() * v1.getZ())
                .setY(v1.getZ() * v2.getX() - v2.getZ() * v1.getX())
                .setZ(v1.getX() * v2.getY() - v2.getX() * v1.getY());
    }

    public static float vec3Dot(GLVector3D v1, GLVector3D v2){
        return v1.getX() * v2.getX() + v1.getY() * v2.getY() + v1.getZ() * v2.getZ();
    }

    public static GLRay point2Ray(float x, float y, GL360Director director){
        GLVector3D v = new GLVector3D();
        ...
        if (success){
            GLVector3D vPickRayDir = new GLVector3D();
            GLVector3D vPickRayOrig = new GLVector3D();
            ...
            return new GLRay(vPickRayOrig,vPickRayDir);
        } else {
            return null;
        }
    }

    public static boolean intersectTriangle(GLRay ray, GLVector3D v0, GLVector3D v1, GLVector3D v2){
        // Find vectors for two edges sharing vert0
        GLVector3D edge1 = vec3Sub(v1 , v0);
        GLVector3D edge2 = vec3Sub(v2 , v0);
